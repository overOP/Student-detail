    ...
    formsContainer.innerHTML = `
        <form id="studentForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="studentId" value="${student.id}" required ${editIndex !== null ? 'readonly' : ''}>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="studentClass" value="${student.class}" required>
            </div>
            <div class="form-actions">
                <button type="submit">${editIndex !== null ? 'Update' : 'Save'} Student</button>
    ...
    formsContainer.innerHTML = `
        <form id="marksForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="marksSubject" value="${markData.subject}" required>
            </div>
            <div class="form-group">
                <label for="marksScore">Marks (out of 100):</label>
                <input type="number" id="marksScore" min="0" max="100" value="${markData.score}" required>
            </div>
            <div class="form-actions">
                <button type="submit">${editIndex !== null ? 'Update' : 'Save'} Marks</button>
                ${editIndex !== null ? '<button type="button" onclick="cancelEdit()">Cancel</button>' : ''}
            </div>
        </form>
    `;
...
function showSortedPerformance() {
    ...
    for (const studentId in studentScores) {
        const avgScore = studentScores[studentId].total / studentScores[studentId].count;
...
window.deleteStudent = function(index) {
    if (confirm('Are you sure you want to delete this student? This will also delete all related attendance and marks records.')) {
        const students = getStudents();
        ...
        const attendance = getAttendance().filter(record => record.studentId !== studentId);
        saveAttendance(attendance);
        
        // Delete related academic marks
        const marks = getAcademicMarks().filter(mark => mark.studentId !== studentId);
        saveAcademicMarks(marks);
...
window.editAttendance = function(index) {
    ...
    let options = students.map(student => 
        `<option value="${student.id}" ${student.id === record.studentId ? 'selected' : ''}>${student.name} (${student.id})</option>`
    ).join('');
    ...
    formsContainer.innerHTML = `
        <form id="editAttendanceForm">
            ...
            <div class="form-group">
                ...
                <input type="date" id="editAttendanceDate" value="${record.date}" required>
            </div>
            <div class="form-group">
